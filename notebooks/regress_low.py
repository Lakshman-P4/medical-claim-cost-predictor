# ============================================================
# LOW COMPLEXITY REGRESSION - ACTUAL LABELS (WITH HYBRID)
# ============================================================
...
print("="*70)
print("LOW COMPLEXITY REGRESSION - ACTUAL LABELS")
print("="*70)
...
# ============================================================
# STEP 3: GET LOW CLAIMS USING ACTUAL LABELS
# ============================================================
print("\n" + "="*70)
print("STEP 3: GET LOW CLAIMS (ACTUAL LABELS)")
print("="*70)

# USE ACTUAL COMPLEXITY LABELS - NOT ROUTER!
low_claim_ids = df_claims[df_claims['COMPLEXITY'] == 'LOW']['CLAIM_ID'].values
print(f"LOW claims (ACTUAL): {len(low_claim_ids):,}")

low_costs = df_claims[df_claims['COMPLEXITY'] == 'LOW']['TOTAL_CLAIM_COST']
print(f"\nLOW Cost Statistics (ACTUAL):")
print(f"  Min:    ${low_costs.min():,.2f}")
print(f"  Max:    ${low_costs.max():,.2f}")
print(f"  Mean:   ${low_costs.mean():,.2f}")
print(f"  Median: ${low_costs.median():,.2f}")

df_low = df[df['CLAIM_ID'].isin(low_claim_ids)].copy()
print(f"LOW visits: {len(df_low):,}")
...
if 'ICD_COUNT' not in df_low.columns:
    if 'unique_icd_codes_count' in df_low.columns:
        df_low['ICD_COUNT'] = df_low['unique_icd_codes_count']
    elif 'has_ICD' in df_low.columns:
        df_low['ICD_COUNT'] = df_low['has_ICD'].astype(int)
    else:
        df_low['ICD_COUNT'] = 0

for col in label_encoders:
    enc_col = f'{col}_ENC'
    if col in df_low.columns and enc_col not in df_low.columns:
        le = label_encoders[col]
        df_low[enc_col] = df_low[col].fillna('UNKNOWN').astype(str).apply(
            lambda x: le.transform([x])[0] if x in le.classes_ else 0
        )

sequence_features_available = [f for f in sequence_features if f in df_low.columns]
print(f"Sequence features: {len(sequence_features_available)}")

for col in sequence_features_available:
    df_low[col] = df_low[col].fillna(0)
...
for claim_id, group in df_low.groupby('CLAIM_ID'):
...
results['Hybrid'] = {
    'model': None,  # No single model for hybrid
...
# Save best single model (not hybrid)
best_single = max([k for k in results if k != 'Hybrid'], key=lambda x: results[x]['test_r2'])

# If hybrid is best, save the top model from hybrid ensemble
if best_name == 'Hybrid':
    # Save top 3 models for hybrid
    hybrid_models = {}
    for model_name in results['Hybrid']['ensemble_models']:
        hybrid_models[model_name] = results[model_name]['model']
    joblib.dump(hybrid_models, MODEL_PATH / 'low_model.joblib')
    print(f"Saved: {MODEL_PATH / 'low_model.joblib'} (Hybrid: {results['Hybrid']['ensemble_models']})")
else:
    joblib.dump(results[best_name]['model'], MODEL_PATH / 'low_model.joblib')
    print(f"Saved: {MODEL_PATH / 'low_model.joblib'} ({best_name})")
...
with open(MODEL_PATH / 'low_artifacts.pkl', 'wb') as f:
    pickle.dump(low_artifacts, f)
print(f"Saved: {MODEL_PATH / 'low_artifacts.pkl'}")
...
fig.suptitle(f'LOW Regression (ACTUAL Labels)\nBest: {best_name} (R² = {best_r2:.4f})', 
...
ax2.scatter(y_test, y_pred_best, alpha=0.3, s=10, c='#2ecc71')
...
# 4. Cost Distribution
ax4 = axes[1, 1]
ax4.hist(y, bins=50, color='#2ecc71', edgecolor='white', alpha=0.7)
...
ax4.set_title(f'LOW Cost Distribution (n={len(y):,})')
ax4.legend()

plt.tight_layout()
plt.savefig(MODEL_PATH / 'low_results_actual.png', dpi=150, bbox_inches='tight')
plt.close()
print(f"Saved: {MODEL_PATH / 'low_results_actual.png'}")

# ============================================================
# SUMMARY
# ============================================================
print("\n" + "="*70)
print("LOW MODEL COMPLETE (ACTUAL LABELS)")
print("="*70)
...
║                    LOW MODEL SUMMARY                              ║
╠══════════════════════════════════════════════════════════════════╣
║  Training Data: ACTUAL LOW labels (not router)                    ║
║  Claims:        {len(low_claim_ids):>10,}                                       ║
...
FILES SAVED:
  1. {MODEL_PATH / 'low_model.joblib'}
  2. {MODEL_PATH / 'low_artifacts.pkl'}
  3. {MODEL_PATH / 'low_results_actual.png'}

NEXT: Run train_med_actual.py
""")