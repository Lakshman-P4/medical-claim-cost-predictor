# ============================================================
# MED COMPLEXITY REGRESSION - ACTUAL LABELS (WITH HYBRID)
# ============================================================
...
print("="*70)
print("MED COMPLEXITY REGRESSION - ACTUAL LABELS")
print("="*70)
...
# ============================================================
# STEP 3: GET MED CLAIMS USING ACTUAL LABELS
# ============================================================
print("\n" + "="*70)
print("STEP 3: GET MED CLAIMS (ACTUAL LABELS)")
print("="*70)

# USE ACTUAL COMPLEXITY LABELS - NOT ROUTER!
med_claim_ids = df_claims[df_claims['COMPLEXITY'] == 'MED']['CLAIM_ID'].values
print(f"MED claims (ACTUAL): {len(med_claim_ids):,}")

med_costs = df_claims[df_claims['COMPLEXITY'] == 'MED']['TOTAL_CLAIM_COST']
print(f"\nMED Cost Statistics (ACTUAL):")
print(f"  Min:    ${med_costs.min():,.2f}")
print(f"  Max:    ${med_costs.max():,.2f}")
print(f"  Mean:   ${med_costs.mean():,.2f}")
print(f"  Median: ${med_costs.median():,.2f}")

df_med = df[df['CLAIM_ID'].isin(med_claim_ids)].copy()
print(f"MED visits: {len(df_med):,}")
...
if 'ICD_COUNT' not in df_med.columns:
    if 'unique_icd_codes_count' in df_med.columns:
        df_med['ICD_COUNT'] = df_med['unique_icd_codes_count']
    elif 'has_ICD' in df_med.columns:
        df_med['ICD_COUNT'] = df_med['has_ICD'].astype(int)
    else:
        df_med['ICD_COUNT'] = 0

for col in label_encoders:
    enc_col = f'{col}_ENC'
    if col in df_med.columns and enc_col not in df_med.columns:
        le = label_encoders[col]
        df_med[enc_col] = df_med[col].fillna('UNKNOWN').astype(str).apply(
            lambda x: le.transform([x])[0] if x in le.classes_ else 0
        )

sequence_features_available = [f for f in sequence_features if f in df_med.columns]
print(f"Sequence features: {len(sequence_features_available)}")

for col in sequence_features_available:
    df_med[col] = df_med[col].fillna(0)
...
for claim_id, group in df_med.groupby('CLAIM_ID'):
...
if best_name == 'Hybrid':
    ...
    joblib.dump(hybrid_models, MODEL_PATH / 'med_model.joblib')
    print(f"Saved: {MODEL_PATH / 'med_model.joblib'} (Hybrid: {results['Hybrid']['ensemble_models']})")
else:
    joblib.dump(results[best_name]['model'], MODEL_PATH / 'med_model.joblib')
    print(f"Saved: {MODEL_PATH / 'med_model.joblib'} ({best_name})")
...
with open(MODEL_PATH / 'med_artifacts.pkl', 'wb') as f:
    pickle.dump(med_artifacts, f)
print(f"Saved: {MODEL_PATH / 'med_artifacts.pkl'}")
...
fig.suptitle(f'MED Regression (ACTUAL Labels)\nBest: {best_name} (R² = {best_r2:.4f})', 
...
colors = ['#f39c12' if m == best_name else '#3498db' for m in model_names]
...
ax2.scatter(y_test, y_pred_best, alpha=0.3, s=10, c='#f39c12')
...
# 4. Cost Distribution
ax4 = axes[1, 1]
ax4.hist(y, bins=50, color='#f39c12', edgecolor='white', alpha=0.7)
...
ax4.set_title(f'MED Cost Distribution (n={len(y):,})')
ax4.legend()

plt.tight_layout()
plt.savefig(MODEL_PATH / 'med_results_actual.png', dpi=150, bbox_inches='tight')
plt.close()
print(f"Saved: {MODEL_PATH / 'med_results_actual.png'}")

# ============================================================
# SUMMARY
# ============================================================
print("\n" + "="*70)
print("MED MODEL COMPLETE (ACTUAL LABELS)")
print("="*70)
...
║                    MED MODEL SUMMARY                              ║
╠══════════════════════════════════════════════════════════════════╣
║  Training Data: ACTUAL MED labels (not router)                    ║
║  Claims:        {len(med_claim_ids):>10,}                                       ║
...
FILES SAVED:
  1. {MODEL_PATH / 'med_model.joblib'}
  2. {MODEL_PATH / 'med_artifacts.pkl'}
  3. {MODEL_PATH / 'med_results_actual.png'}

NEXT: Run train_high_actual.py
""")